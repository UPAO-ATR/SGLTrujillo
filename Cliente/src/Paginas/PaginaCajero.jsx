...
import { Area, Boton, Campo, EtiquetaEstado, Mensaje, Seccion, Selector } from "../Componentes/Comunes.jsx";
...
function Renovacion({ alCompletar }) {
  ...
  return <Seccion titulo="Renovación de licencia"><Mensaje>{Mensaje}</Mensaje><div className="fila-accion"><Campo etiqueta="RUC" value={Ruc} maxLength="11" onChange={(e) => DefinirRuc(e.target.value.replace(/\D/g,""))} /><Boton onClick={Buscar}>Buscar licencias</Boton></div>{Licencias.length > 0 && <div className="tarjetas">{Licencias.map((Item) => <button key={Item.id} className={`tarjeta seleccionable ${Elegida?.id === Item.id ? "seleccionada" : ""}`} onClick={() => DefinirElegida(Item)}><b>{Item.direccion}</b><EtiquetaEstado estado={Item.estado} /><span>Vence: {Fecha(Item.fecha_vencimiento)}</span></button>)}</div>}{Elegida?.estado === "APROBADO" && <Mensaje tipo="advertencia">La licencia sigue activa y vence el {Fecha(Elegida.fecha_vencimiento)}. No se permite renovarla todavía.</Mensaje>}{Elegida?.estado === "VENCIDO" && !Completada && <><div className="rejilla dos"><Campo etiqueta="DNI del titular" value={Dni} maxLength="8" onChange={(e) => DefinirDni(e.target.value.replace(/\D/g,""))} /><Campo etiqueta="Fecha de nacimiento complementaria" type="date" value={FechaNacimiento} onChange={(e) => DefinirFechaNacimiento(e.target.value)} /><Campo etiqueta="Correo de contacto" type="email" value={Correo} onChange={(e) => DefinirCorreo(e.target.value)} placeholder={Elegida.correo} /></div><PagoDemostrativo codigo={Elegida.codigo} bloqueado={Dni.length !== 8} alPagar={Renovar} /></>}{Completada && <><Mensaje tipo="exito">{Mensaje}</Mensaje><Boton onClick={() => AbrirArchivoProtegido(`/cajero/comprobantes/${Completada.TramiteId}`)}>Abrir boleta demostrativa</Boton></>}</Seccion>;
}
